...
public class NBClassifier {
	public static void train(String trainingData, String modelFile) throws Exception {
		if ((new File(modelFile)).exists()) {
	        ...
	        FileOutputFormat.setOutputPath(job, new Path("hdfs://localhost:9005/down"));
	        job.waitForCompletion(true);
	      //下载模型文件数据
		
			URI uri = new URI("hdfs://localhost:9005") ;
			FileSystem file = FileSystem.get(uri, conf);
			Path src = new Path("hdfs://localhost:9005/down/part-r-00000") ;
	...
	public static void main(String[] args) throws Exception {
		...
		String modelFile = "E://hadoop/part-r-00000"; // 这里指定model文件将放置在本地的那个目录下，并给model文件命名
